...
def mic_cal(stream):
    ...
    for n in range(4):
        ...
        for i in range((int) (CAL_TIME_EACH_COMMAND * COLLECT_FREQUENCY)):
            main_freq = get_main_freq(stream)
...
def main():
    ...
    # Debugging
    #while True:
    #    print(get_main_freq(stream))
    cal_avgs, cal_stds = mic_cal(stream)
    mic_run(stream, cal_avgs, cal_stds)
    mic_close(stream)
